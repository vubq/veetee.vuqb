...
|---|---|---|
| M0 | Host protocol/provider path, firmware build/serial và runtime turn đã chạy; database/provider config có revision; unattended wake đã đưa được audio qua pipeline; physical harness đã ghi đủ 30/30 repetition (120 event) | Người dùng xác nhận nghe loa/nhìn LCD/bấm PTT và acceptance audio path không rơi frame |
| M1 | WS v1/v2/v3 compatibility fixtures, streaming/cancellation/admission, WakeNet init, wake→TTS exact image và 30-turn unattended wake lifecycle đã pass | AFE/AEC/noise, barge-in, false accept/reject corpus, 60-minute physical soak, warm TTFA/provider promotion, encoder warm-up và cross-peer physical conformance |
