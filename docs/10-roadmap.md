...
## 1.1 Trạng thái evidence hiện tại

| Milestone | Đã chứng minh | Còn mở trước khi đánh dấu đạt |
|---|---|---|
| M0 | Host protocol/provider path, firmware build/serial và runtime turn đã chạy; database/provider config có revision. Các report 30/30 và 94 repetition là **auto wake evidence của M1**, không thay thế manual PTT acceptance của M0. | Manual PTT phải chứng minh `idle → connecting → listening → speaking → idle`, transcript acceptance, người dùng nghe loa/nhìn LCD và 30 turn không rơi frame/reboot/deadlock. |
| M1 | WS v1/v2/v3 compatibility fixtures, streaming/cancellation/admission, WakeNet init, wake→TTS exact image, split encoder/decoder lock, 30-turn unattended wake lifecycle, host warm TTFA p95 `1377,5 ms`, fixture physical soak hơn 60 phút, AEC adapter lifecycle/resource evidence (10/10 fixture repetitions), stale `tts/stop` barrier, normal wake 2-lượt và physical barge-in lifecycle đã pass | Acoustic AEC echo-only quality, false accept/reject corpus, voice-onset barge-in/time-to-silence, 100-repetition completion, provider artifact/resource promotion và cross-peer physical conformance |
...
| M3 | MQTT control config/codec/session gate, internal gateway bridge codec, host-only session coordinator, bounded asyncio UDP datagram carrier boundary, explicit host-only gateway composition, firmware UDP header/crypto codec, UDP v3 framing/AES-CTR/bounded-reorder, deterministic loss/reorder soak, ordering barrier `tts/start`/`tts/stop` và BoardHal logical capability manifest validation đã có host-only/ESP-IDF golden/test; chưa đưa carrier vào Voice runtime critical path | broker/firmware carrier live wiring, real-network loss/latency comparison, BOM/descriptor thật/peripheral owner tools, hardware MCP, assets/OTA và transport-promotion evidence |
| M4 | Chưa bắt đầu promotion | Locale thứ hai, capacity/backup/security/soak gates |

Bảng này chỉ là trạng thái evidence, không gắn ngày hoặc thời lượng cho milestone.
