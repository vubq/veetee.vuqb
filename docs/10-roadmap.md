...
| M3 | UDP v3 framing/AES-CTR/bounded-reorder và ordering barrier `tts/start`/`tts/stop` đã có host-only golden/test; chưa đưa carrier vào runtime critical path | MQTT client/gateway/socket, firmware carrier, loss/soak, BOM/descriptor/safety policy, hardware MCP, assets/OTA và transport-promotion evidence |
