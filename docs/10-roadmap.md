...
| M3 | MQTT control config/codec/session gate, internal gateway bridge codec, host-only session coordinator, UDP v3 framing/AES-CTR/bounded-reorder, deterministic loss/reorder soak và ordering barrier `tts/start`/`tts/stop` đã có host-only golden/test; chưa đưa carrier vào runtime critical path | MQTT client/gateway/socket, firmware carrier, real-network loss/latency comparison, BOM/descriptor/safety policy, hardware MCP, assets/OTA và transport-promotion evidence |
