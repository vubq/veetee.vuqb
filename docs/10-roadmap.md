...
|---|---|---|
| M0 | Host protocol/provider path, firmware build/serial và runtime turn đã chạy; database/provider config có revision; unattended wake đã đưa được audio qua pipeline; physical harness đã ghi ít nhất 30/30 repetition (một lượt mới đạt 94 repetition hoàn chỉnh), không còn lỗi capture encode sau khi tách lock | Người dùng xác nhận nghe loa/nhìn LCD/bấm PTT và acceptance audio path không rơi frame |
| M1 | WS v1/v2/v3 compatibility fixtures, streaming/cancellation/admission, WakeNet init, wake→TTS exact image, split encoder/decoder lock, 30-turn unattended wake lifecycle, host warm TTFA p95 `1377,5 ms`, fixture physical soak hơn 60 phút, và AEC adapter lifecycle/resource evidence (10/10 fixture repetitions) đã pass | Acoustic AEC echo-only quality, false accept/reject corpus, voice-onset barge-in/time-to-silence, 100-repetition completion, provider artifact/resource promotion và cross-peer physical conformance |
