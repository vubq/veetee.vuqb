...
| M3 | MQTT control config/codec/session gate, internal gateway bridge codec, host-only session coordinator, firmware UDP header codec, UDP v3 framing/AES-CTR/bounded-reorder, deterministic loss/reorder soak, ordering barrier `tts/start`/`tts/stop` và BoardHal logical capability manifest validation đã có host-only/ESP-IDF golden/test; chưa đưa carrier vào runtime critical path | MQTT client/gateway/socket, firmware encrypted carrier, real-network loss/latency comparison, BOM/descriptor thật/peripheral owner tools, hardware MCP, assets/OTA và transport-promotion evidence |
