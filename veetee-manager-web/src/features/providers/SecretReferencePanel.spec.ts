...
describe('SecretReferencePanel', () => {
  it('keeps secret value write-only and emits provider selection changes', async () => {
    ...
    const name = view.getByRole('textbox', { name: 'Tên secret' })
    const value = view.getByLabelText('Secret value')
    await fireEvent.update(name, 'New secret')
    await fireEvent.update(value, 'do-not-render')
    await fireEvent.click(view.getByRole('button', { name: 'Lưu secret' }))
  ...
  it('rotates a selected reference and clears the new value after success', async () => {
    ...
    await fireEvent.click(view.getByRole('button', { name: 'Rotate' }))
    const value = await view.findByLabelText('Secret value mới')
    await fireEvent.update(value, 'rotated-only-on-request')
    await fireEvent.click(view.getByRole('button', { name: 'Rotate secret' }))
  ...
  it('keeps the reference when deletion is rejected by the gateway', async () => {
    ...
    await fireEvent.click(view.getByRole('button', { name: 'Xóa secret' }))
    expect((await view.findByRole('alert')).textContent).toContain('Secret đang được provider revision sử dụng')
    expect(view.getByText('Groq test key')).toBeTruthy()
  })
})
