...
export async function buildApp(overrides?: { env?: Environment; store?: Store }): Promise<FastifyInstance> {
  ...
  app.get('/health/ready', async (_request, reply) => {
    const publication = store.runtime()
  ...
  app.get('/api/v1/provider-installations', async () => ({ items: store.listInstallations() }))
  app.get<{ Querystring: { locale?: string } }>('/api/v1/voices', async (request) => ({ items: store.listInstallations().filter((item) => item.kind === 'tts' && (!request.query.locale || item.manifest.locales === undefined || (item.manifest.locales as unknown[]).includes('*') || (item.manifest.locales as unknown[]).includes(request.query.locale))).map((item) => ({ id: item.id, name: item.displayNameKey, providerName: item.displayNameKey, locale: request.query.locale ?? '*', description: item.displayNameKey, previewDurationMs: 0, available: true })), total: store.listInstallations().filter((item) => item.kind === 'tts').length }))
  app.get<{ Querystring: { kind?: ProviderKind } }>('/api/v1/provider-configs', async (request: FastifyRequest<{ Querystring: { kind?: ProviderKind } }>) => ({ items: store.listProviderConfigs(owner(request), request.query.kind) }))
  app.post<{ Body: { installationId: string; name: string; config: Record<string, unknown>; secretRefs?: string[] } }>('/api/v1/provider-configs', { schema: { body: providerBodySchema } }, async (request, reply) => {
    try {
      const value = store.createProviderConfig(owner(request), request.body)
  ...
  app.patch<{ Params: { id: string }; Body: { name?: string; config?: Record<string, unknown>; secretRefs?: string[] } }>('/api/v1/provider-configs/:id', { schema: { body: { type: 'object', additionalProperties: false, properties: { name: { type: 'string', minLength: 1, maxLength: 80 }, config: { type: 'object' }, secretRefs: { type: 'array', items: { type: 'string' } } } } } }, async (request, reply) => {
    ...
    try { const value = store.updateProviderConfig(owner(request), request.params.id, request.body, ifMatch); return reply.header('ETag', value.etag).send(value) } catch (error) { return sendProblem(reply, error) }
  })

  app.get('/api/v1/assistants', async (request) => ({ items: store.listAssistants(owner(request)) }))
  app.post<{ Body: { name: string } }>('/api/v1/assistants', { schema: { body: assistantBodySchema } }, async (request, reply) => reply.code(201).send(store.createAssistant(owner(request), request.body.name)))
  app.get<{ Params: { id: string } }>('/api/v1/assistants/:id', async (request, reply) => {
    const item = store.getAssistant(owner(request), request.params.id)
  ...
  app.patch<{ Params: { id: string }; Body: Record<string, unknown> }>('/api/v1/assistants/:id/role-config', { schema: { body: roleBodySchema } }, async (request, reply) => {
    const ifMatch = request.headers['if-match']
    if (typeof ifMatch !== 'string') return reply.code(428).send({ code: 'IF_MATCH_REQUIRED' })
    try { const item = store.updateRole(owner(request), request.params.id, request.body, ifMatch); return reply.header('ETag', item.etag).send(item.role) } catch (error) { return sendProblem(reply, error) }
  })
  app.get<{ Params: { id: string } }>('/api/v1/assistants/:id/model-memory', async (request, reply) => {
    try {
      const item = store.getAssistant(owner(request), request.params.id)
      if (!item) return reply.code(404).send({ code: 'NOT_FOUND' })
      return reply.header('ETag', item.etag).send(store.getModelMemory(owner(request), request.params.id))
    } catch (error) { return sendProblem(reply, error) }
  })
  app.patch<{ Params: { id: string }; Body: { kind: ProviderKind; mode: 'selected' | 'disabled'; providerConfigId?: string } }>('/api/v1/assistants/:id/model-memory/provider', { schema: { body: { type: 'object', additionalProperties: false, required: ['kind', 'mode'], properties: { kind: { type: 'string', enum: ['vad', 'asr', 'llm', 'tts', 'intent', 'memory'] }, mode: { type: 'string', enum: ['selected', 'disabled'] }, providerConfigId: { type: 'string' } } } } }, async (request, reply) => {
    const ifMatch = request.headers['if-match']
    if (typeof ifMatch !== 'string') return reply.code(428).send({ code: 'IF_MATCH_REQUIRED' })
    try { const value = store.updateProviderSelection(owner(request), request.params.id, request.body, ifMatch); const assistant = store.getAssistant(owner(request), request.params.id); return reply.header('ETag', assistant?.etag ?? '').send(value) } catch (error) { return sendProblem(reply, error) }
  })
  app.patch<{ Params: { id: string }; Body: { enabled: boolean } }>('/api/v1/assistants/:id/model-memory/memory', { schema: { body: { type: 'object', additionalProperties: false, required: ['enabled'], properties: { enabled: { type: 'boolean' } } } } }, async (request, reply) => {
    const ifMatch = request.headers['if-match']
    if (typeof ifMatch !== 'string') return reply.code(428).send({ code: 'IF_MATCH_REQUIRED' })
    try { const value = store.setMemoryEnabled(owner(request), request.params.id, request.body.enabled, ifMatch); const assistant = store.getAssistant(owner(request), request.params.id); return reply.header('ETag', assistant?.etag ?? '').send(value) } catch (error) { return sendProblem(reply, error) }
  })
  app.post<{ Params: { id: string } }>('/api/v1/assistants/:id/publish', async (request, reply) => {
    try { const publication = store.publish(owner(request), request.params.id, typeof request.headers['if-match'] === 'string' ? request.headers['if-match'] : undefined); return reply.header('ETag', publication.etag).send(publication) } catch (error) { return sendProblem(reply, error) }
  })
  app.get<{ Params: { id: string } }>('/api/v1/assistants/:id/devices', async (request, reply) => {
    if (!store.getAssistant(owner(request), request.params.id)) return reply.code(404).send({ code: 'NOT_FOUND' })
  ...
  app.get('/internal/v1/runtime-config', async (request, reply) => {
...
async function createStore(env: Environment): Promise<Store> {
  ...
  if (env.VEETEE_DATABASE_MODE === 'postgres') throw new Error('PostgreSQL adapter is not enabled in this build; use migration-backed adapter before selecting postgres')
